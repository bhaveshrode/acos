...
export class AuthenticationService {
  ...
  private readonly sessionLifespanHours: number;
  private readonly refreshLifespanDays: number;

  constructor(
    passwordHasher: IPasswordHasher,
    sessionLifespanHours: number = 2,
    refreshLifespanDays: number = 7
  ) {
    ...
    if (user.status === "SUSPENDED") {
      return Result.fail(
        ResultError.unauthorized(
          "Authentication failed. Account has been suspended due to consecutive failed attempts."
        )
      );
    }
