...
import { GatewayReference } from "../value-objects/GatewayReference.js";
...
export class Payment extends AggregateRoot<PaymentId> {
  ...
  public get amount(): PaymentAmount { return this.props.amount; }
  ...
  public get gatewayReference(): GatewayReference | null { return this.props.gatewayReference; }
  public get walletAddress(): WalletAddress | null { return this.props.walletAddress; }
  public get metadata(): PaymentMetadata { return this.props.metadata; }
  public get confirmations(): ConfirmationCount { return this.props.confirmations; }
  public get exchangeRate(): ExchangeRate | null { return this.props.exchangeRate; }
  public get allocations(): readonly PaymentAllocation[] { return Object.freeze(Array.from(this.props.allocations.values())); }
  public get attempts(): readonly PaymentAttempt[] { return Object.freeze([...this.props.attempts]); }
  public get refundRequests(): readonly RefundRequest[] { return Object.freeze(Array.from(this.props.refundRequests.values())); }
   ...
   */
  public submit(attemptId: UniqueEntityID): Result<void> {
   ...
   */
  public startProcessing(attemptId: UniqueEntityID, gatewayRef: GatewayReference): Result<void> {
    const editCheck = this.ensureMutable();
    if (editCheck.isFailure) return Result.fail(editCheck.error);
  ...
  public confirm(
    ...
    if (this.status === PaymentStatus.CONFIRMED) return Result.ok();
   ...
   */
  public fail(attemptId: UniqueEntityID, errorCode: string, errorMessage: string): Result<void> {
    if (this.status === PaymentStatus.CONFIRMED) {
   ...
   */
  public requestRefund(requestId: UniqueEntityID, amount: Money, reason: string): Result<void> {
   ...
   */
  public completeRefund(requestId: UniqueEntityID): Result<void> {
    ...
    if (refund.status === RefundStatus.COMPLETED) return Result.ok();
   ...
   */
  public approveRefund(requestId: UniqueEntityID): Result<void> {
    const refund = this.props.refundRequests.get(requestId.value);
    if (!refund) {
      return Result.fail(ResultError.notFound("Refund request not found."));
    }
   ...
   */
  public rejectRefund(requestId: UniqueEntityID): Result<void> {
    const refund = this.props.refundRequests.get(requestId.value);
    if (!refund) {
      return Result.fail(ResultError.notFound("Refund request not found."));
    }
