...
export class ReceivablePolicy {
   ...
   */
  public validateWriteOffPermissions(
    ...
    if (amount.amount > outstanding.amount) {
      return Result.fail(
        ResultError.conflict(
          `Write-off amount ${amount.amount} exceeds net outstanding debt ${outstanding.amount}.`
        )
      );
    }
   ...
   */
  public validateAccountClosure(ar: AccountsReceivable): Result<void> {
    const hasOutstanding = ar.entries.some((entry) => !entry.isPaid);
    if (hasOutstanding) {
      return Result.fail(
        ResultError.conflict("Cannot close a receivable account with outstanding debt.")
      );
    }
    return Result.ok();
  }
   ...
   */
  public validateAccountReopening(ar: AccountsReceivable): Result<void> {
    return Result.ok();
  }
}
