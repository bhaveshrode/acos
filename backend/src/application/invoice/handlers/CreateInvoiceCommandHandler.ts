...
import { PaymentTerms, PaymentTermType } from "../../../business/invoice/value-objects/PaymentTerms.js";
...
export class CreateInvoiceCommandHandler
  implements IRequestHandler<CreateInvoiceCommand, ApplicationResult<InvoiceResponseDto>>
{
  constructor(
  ...
  public async handle(
    ...
    if (numRes.isFailure) return ApplicationResult.failure(numRes.error.message);
    ...
    // Parse value objects
    const termsRes = PaymentTerms.create(dto.paymentTerms as PaymentTermType);
    if (termsRes.isFailure) return ApplicationResult.failure(termsRes.error.message);

    const dueDateRes = DueDate.create(new Date(dto.dueDate));
    if (dueDateRes.isFailure) return ApplicationResult.failure(dueDateRes.error.message);

    // Create Invoice aggregate
    const invoiceRes = Invoice.create(
      InvoiceId.generate(),
      orgId,
      customerId,
      numRes.value,
    ...
    if (invoiceRes.isFailure) return ApplicationResult.failure(invoiceRes.error.message);
    ...
    for (const line of dto.lines) {
      ...
      if (priceRes.isFailure) return ApplicationResult.failure(priceRes.error.message);

      const qtyRes = Quantity.create(line.quantity);
      if (qtyRes.isFailure) return ApplicationResult.failure(qtyRes.error.message);

      const taxRes = TaxRate.create(line.taxRate);
      if (taxRes.isFailure) return ApplicationResult.failure(taxRes.error.message);

      const addLineRes = invoice.addLineItem(
        new UniqueEntityID(),
        line.description,
        qtyRes.value,
        priceRes.value,
      ...
      if (addLineRes.isFailure) return ApplicationResult.failure(addLineRes.error.message);
    }
