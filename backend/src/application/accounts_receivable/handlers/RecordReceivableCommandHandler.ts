...
export class RecordReceivableCommandHandler
  implements IRequestHandler<RecordReceivableCommand, ApplicationResult<ReceivableResponseDto>>
{
  constructor(
  ...
  public async handle(
    ...
    if (findRes.isSuccess && findRes.value) {
      ...
      const createRes = AccountsReceivable.create(
        ReceivableAccountId.generate(),
        orgId,
        customerId
      );
      if (createRes.isFailure) return ApplicationResult.failure(createRes.error.message);
      ar = createRes.value;
    }

    // Add invoice entry to customer obligations
    const entryRes = ar.addInvoice(
      new UniqueEntityID(),
      invoiceId,
      moneyRes.value,
    ...
    if (entryRes.isFailure) return ApplicationResult.failure(entryRes.error.message);
