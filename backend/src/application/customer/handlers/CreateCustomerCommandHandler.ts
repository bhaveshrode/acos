...
export class CreateCustomerCommandHandler
  implements IRequestHandler<CreateCustomerCommand, ApplicationResult<CustomerResponseDto>>
{
  constructor(
  ...
  public async handle(request: CreateCustomerCommand): Promise<ApplicationResult<CustomerResponseDto>> {
    const { dto } = request;
    ...
    if (custNumRes.isFailure) return ApplicationResult.failure(custNumRes.error.message);
    ...
    if (existsRes.value) {
      return ApplicationResult.failure(
        `Customer number '${dto.customerNumber}' already exists in this organization.`
      );
    }
    ...
    if (contactEmailRes.isFailure) return ApplicationResult.failure(contactEmailRes.error.message);
    ...
    const billingAddressRes = Address.create(
    ...
    if (billingAddressRes.isFailure) return ApplicationResult.failure(billingAddressRes.error.message);
    ...
    if (customerRes.isFailure) return ApplicationResult.failure(customerRes.error.message);
