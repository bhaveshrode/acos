...
export class CreateOrganizationCommandHandler {
    ...
    async handle(request) {
        ...
        if (slugRes.isFailure)
            return ApplicationResult.failure(slugRes.error.message);
        ...
        if (existsRes.value) {
            return ApplicationResult.failure(`Organization with slug '${dto.slug}' already exists.`);
        }
        const ownerId = UserId.from(dto.ownerId);
        // Set defaults
        const currencyRes = Currency.create(dto.currency || "USD");
        if (currencyRes.isFailure)
            return ApplicationResult.failure(currencyRes.error.message);
        const timeZoneRes = TimeZone.create("UTC");
        if (timeZoneRes.isFailure)
            return ApplicationResult.failure(timeZoneRes.error.message);
        const settingsRes = OrganizationSettings.create(currencyRes.value, timeZoneRes.value);
        if (settingsRes.isFailure)
            return ApplicationResult.failure(settingsRes.error.message);
        // Create Organization
        const organization = Organization.create(OrganizationId.generate(), nameRes.value, slugRes.value, ownerId, settingsRes.value);
