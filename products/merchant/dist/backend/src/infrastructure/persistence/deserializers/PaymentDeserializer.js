...
export class PaymentDeserializer {
    static deserialize(snapshot) {
        ...
        for (const alloc of snapshot.allocations) {
            allocations.set(alloc.id, new PaymentAllocation(new UniqueEntityID(alloc.id), {
                invoiceId: new InvoiceId(alloc.invoiceId),
                allocatedAmount: Money.create(alloc.amount, alloc.currency).value,
                status: alloc.status
            }));
        }
        const attempts = snapshot.attempts.map((att) => new PaymentAttempt(new UniqueEntityID(att.id), {
        ...
        for (const ref of snapshot.refundRequests) {
            refundRequests.set(ref.id, new RefundRequest(new UniqueEntityID(ref.id), {
        ...
        return {
            ...
            method: PaymentMethod.create(snapshot.methodType, snapshot.methodDetails).value,
