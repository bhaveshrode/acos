...
export class CustomerDeserializer {
    static deserialize(snapshot) {
        ...
        for (const addr of snapshot.addresses) {
            const addressVal = Address.create(addr.line1, addr.city, addr.state, addr.country, addr.postalCode, addr.line2 || undefined).value;
            addresses.set(addr.id, new AddressRecord(new UniqueEntityID(addr.id), {
                address: addressVal,
                type: addr.type
            }));
        }
        ...
        for (const c of snapshot.contacts) {
            contacts.set(c.id, new Contact(new UniqueEntityID(c.id), {
                name: c.name,
        ...
        for (const n of snapshot.notes) {
            notes.set(n.id, new CustomerNote(new UniqueEntityID(n.id), {
                content: n.content,
        ...
        return {
            ...
            contacts,
            notes,
            communicationPreferences: CommunicationPreferences.create(snapshot.communicationPreferences.emailEnabled, snapshot.communicationPreferences.smsEnabled, snapshot.communicationPreferences.pushEnabled).value,
