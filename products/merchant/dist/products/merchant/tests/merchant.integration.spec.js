...
describe("Merchant Core Integration Spec Suite (Phase 5 Combined)", () => {
    ...
    beforeEach(() => {
        ...
        if (typeof boundary.userRepository.clear === "function")
            boundary.userRepository.clear();
        if (typeof boundary.organizationRepository.clear === "function")
            boundary.organizationRepository.clear();
        if (typeof boundary.customerRepository.clear === "function")
            boundary.customerRepository.clear();
        if (typeof boundary.invoiceRepository.clear === "function")
            boundary.invoiceRepository.clear();
        if (typeof boundary.paymentRepository.clear === "function")
            boundary.paymentRepository.clear();
        if (typeof boundary.accountsReceivableRepository.clear === "function")
            boundary.accountsReceivableRepository.clear();
        boundary.paymentProvider.clear();
    ...
    describe("E2E Authentication Workflows", () => {
        ...
        it("should sign up a new merchant user successfully through ACOS Identity", async () => {
            ...
            const searchRes = await boundary.userRepository.findByEmail(await import("../../../backend/src/business/identity/value-objects/Email.js").then(m => m.Email.create(email).value));
        ...
        it("should reject /auth/me profile access for unauthenticated requests", async () => {
            ...
            frontend.clearSessionToken();
            frontend.sessionToken = "invalid-jwt-signature-claims";
            await expect(frontend.queryMe()).rejects.toThrow(/Unauthorized/);
        });
        it("should log out successfully, terminating session on ACOS and clearing client token", async () => {
            ...
            frontend.sessionToken = cachedToken;
    ...
    describe("Business Onboarding Workflows", () => {
        ...
        it("should successfully onboard a business for authenticated merchant", async () => {
            const biz = await frontend.onboardBusiness("Bob Retail", "bob-retail", "USD", "E-Commerce", "Canada", "info@bobretail.com");
        ...
        it("should retrieve the active business context for authenticated merchant", async () => {
            ...
            await frontend.onboardBusiness("Bob Retail", "bob-retail", "USD", "E-Commerce", "Canada", "info@bobretail.com");
        ...
        it("should prevent onboarding or retrieving business contexts for unauthenticated requests", async () => {
            frontend.clearSessionToken();
            await expect(frontend.onboardBusiness("Ghost Biz", "ghost-biz", "USD", "SaaS", "USA")).rejects.toThrow(/Unauthorized/);
        ...
        it("should block duplicate business onboarding for the same merchant", async () => {
            ...
            await expect(frontend.onboardBusiness("Bob Garage", "bob-garage", "USD")).rejects.toThrow(/Onboarding Blocked/);
        });
        it("should reject onboarding when using a duplicate slug from another user", async () => {
            ...
            await expect(frontend.onboardBusiness("Charlie Shop", "bob-shop", "USD")).rejects.toThrow(/already exists/);
        });
        it("should validate and reject missing parameters during onboarding", async () => {
            await expect(frontend.onboardBusiness("", "valid-slug")).rejects.toThrow(/name/i);
            await expect(frontend.onboardBusiness("Valid Name", "")).rejects.toThrow(/slug/i);
        });
    });
    ...
    describe("Invoice Lifecycle Workflows (Phase 4)", () => {
        ...
        let bobCustomer;
        let charlieCustomer;
        const baseInvoicePayload = (customerId, number) => ({
            customerId,
        ...
        it("should successfully issue a draft invoice (DRAFT -> ISSUED)", async () => {
            ...
            const queryRes = await boundary.invoiceRepository.findById(await import("../../../backend/src/business/invoice/value-objects/InvoiceId.js").then(m => m.InvoiceId.from(created.id)));
    ...
    describe("Payment Sandbox Workflows (Phase 5)", () => {
        ...
        it("should successfully register a payment request for a SENT invoice", async () => {
            ...
            const queryRes = await boundary.paymentRepository.findById(await import("../../../backend/src/business/payment/value-objects/PaymentId.js").then(m => m.PaymentId.from(payment.id)));
            expect(queryRes.isSuccess).toBe(true);
        ...
        it("should successfully confirm payment via webhook callback simulation (success path)", async () => {
            ...
            const reloadedRes = await boundary.paymentRepository.findById(await import("../../../backend/src/business/payment/value-objects/PaymentId.js").then(m => m.PaymentId.from(payment.id)));
    ...
    describe("Payment Reconciliation Workflows (Phase 6)", () => {
        ...
        let bobCustomer;
        let bobInvoice;
        const baseInvoicePayload = (customerId, number, amount = 500) => ({
            customerId,
            invoiceNumber: number,
            currency: "USD",
            paymentTerms: "NET_30",
        ...
        it("should successfully reconcile partial payment (SENT -> PARTIALLY_PAID)", async () => {
            ...
            const arRes = await boundary.accountsReceivableRepository.findByCustomer(await import("../../../backend/src/business/organization/value-objects/OrganizationId.js").then(m => m.OrganizationId.from(invoice.organizationId)), await import("../../../backend/src/business/customer/value-objects/CustomerId.js").then(m => m.CustomerId.from(invoice.customerId)));
        ...
        it("should successfully reconcile full remaining payment across multiple partial payments", async () => {
            ...
            const queryRes1 = await boundary.paymentRepository.findById(await import("../../../backend/src/business/payment/value-objects/PaymentId.js").then(m => m.PaymentId.from(payment1.id)));
            ...
            const queryRes2 = await boundary.paymentRepository.findById(await import("../../../backend/src/business/payment/value-objects/PaymentId.js").then(m => m.PaymentId.from(payment2.id)));
            ...
            const arRes = await boundary.accountsReceivableRepository.findByCustomer(await import("../../../backend/src/business/organization/value-objects/OrganizationId.js").then(m => m.OrganizationId.from(invoiceFinal.organizationId)), await import("../../../backend/src/business/customer/value-objects/CustomerId.js").then(m => m.CustomerId.from(invoiceFinal.customerId)));
        ...
        it("should reject reconciliation when currency mismatch is detected", async () => {
            ...
            alloc.props.allocatedAmount = eurMoney;
            await boundary.paymentRepository.save(paymentAgg);
            // Trigger webhook, expect failure due to currency validation
            await expect(frontend.simulateWebhook(paymentAgg.gatewayReference.value, true)).rejects.toThrow(/currency|Mismatch/i);
        });
        it("should protect reconciliation idempotency and ignore duplicate webhook callbacks", async () => {
            ...
            const arRes = await boundary.accountsReceivableRepository.findByCustomer(await import("../../../backend/src/business/organization/value-objects/OrganizationId.js").then(m => m.OrganizationId.from(invoice1.organizationId)), await import("../../../backend/src/business/customer/value-objects/CustomerId.js").then(m => m.CustomerId.from(invoice1.customerId)));
            expect(arRes.value.getOutstandingBalance("USD").amount).toBe(0);
        ...
        it("should enforce tenant boundary isolation on reconciliation updates", async () => {
            ...
            const queryRes = await boundary.paymentRepository.findById(await import("../../../backend/src/business/payment/value-objects/PaymentId.js").then(m => m.PaymentId.from(bobPayment.id)));
            ...
            bobAlloc.props.invoiceId = await import("../../../backend/src/business/invoice/value-objects/InvoiceId.js").then(m => m.InvoiceId.from(charlieInvoice.id));
            ...
            await expect(frontend.simulateWebhook(bobGatewayRef, true)).rejects.toThrow(/Access Denied|tenant/i);
        });
    });
    ...
    describe("Merchant Dashboard Workflows (Phase 8)", () => {
        ...
        it("should protect dashboard queries against cross-tenant access", async () => {
            ...
            await expect(backend.handleRequest("GET", "/business/dashboard", {}, { Authorization: `Bearer ${frontend.getSessionToken()}` })).resolves.toEqual(expect.objectContaining({
    ...
    describe("Production Hardening — Track 11.6 (Durable Transactional Outbox)", () => {
        it("should write domain events to the outbox database table and publish them asynchronously", async () => {
            ...
            boundary.prismaClient = mockPrisma;
            ...
            await boundary.eventBus.publish(dummyEvent);
            // Wait for background outbox processor to finish processing
            for (let i = 0; i < 100; i++) {
                if (!boundary.isProcessingOutbox && outboxRecords[0]?.processed) {
                    break;
                }
