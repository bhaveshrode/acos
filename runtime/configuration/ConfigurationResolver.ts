...
export class ConfigurationResolver {
  ...
  public resolve(env: string, overrides: Partial<RuntimeConfiguration> = {}): RuntimeConfiguration {
    ...
    const config = new RuntimeConfiguration(
      profile,
      overrides.databaseUrl ?? dbUrl,
      overrides.apiUrl ?? apiUrl,
      overrides.websocketUrl ?? wsUrl,
      overrides.paymentProvider ?? payment,
      overrides.blockchainProvider ?? blockchain
    );
  ...
  private parseProfile(env: string): EnvironmentProfile {
